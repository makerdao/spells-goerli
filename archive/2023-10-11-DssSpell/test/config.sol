...
contract Config {
    ...
    function setValues(address chief) public {
        ...
        //
        spellValues = SpellValues({
            deployed_spell:         address(0xF91bBa5d4eE67d4B06114730EcFA22B1AaeB25d1), // populate with deployed spell if deployed
            deployed_spell_created: 1697045400,          // use `make deploy-info tx=<deployment-tx>` to obtain the timestamp
            deployed_spell_block:   9849166,          // use `make deploy-info tx=<deployment-tx>` to obtain the block number
