...
contract DssSpellTestBase is Config, DssTest {
    ...
    function _testNextCastTime() internal {
        ...
        if (spell.officeHours()) {
            ...
            for(uint256 i = 0; i < 5; i++) {
                ...
                assertEq(spell.nextCastTime(), castTime, "TestError/cast-time-mismatch");

                vm.warp(castTime + 7 hours + 1 seconds); // 21:00:01 UTC
                if (i < 4) {
                    assertEq(
                        spell.nextCastTime(),
                        monday_1400_UTC + (i + 1) * 1 days,
                        "TestError/cast-time-mismatch-next-day"
                    ); // Next day at 14:00 UTC
                } else {
                    assertEq(
                        spell.nextCastTime(),
                        monday_1400_UTC + 7 days,
                        "TestError/cast-time-mismatch-next-monday"
                    ); // Next monday at 14:00 UTC (friday case)
                }
            }
        }
    }
    ...
    function _testUseEta() internal {
        ...
        assertEq(castTime, spell.eta(), "TestError/eta-cast-time-mismatch");
    }
